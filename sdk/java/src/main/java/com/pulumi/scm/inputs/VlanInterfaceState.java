// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.scm.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import com.pulumi.scm.inputs.VlanInterfaceArpArgs;
import com.pulumi.scm.inputs.VlanInterfaceDdnsConfigArgs;
import com.pulumi.scm.inputs.VlanInterfaceDhcpClientArgs;
import com.pulumi.scm.inputs.VlanInterfaceIpArgs;
import java.lang.Integer;
import java.lang.String;
import java.util.List;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class VlanInterfaceState extends com.pulumi.resources.ResourceArgs {

    public static final VlanInterfaceState Empty = new VlanInterfaceState();

    /**
     * ARP configuration
     * 
     */
    @Import(name="arps")
    private @Nullable Output<List<VlanInterfaceArpArgs>> arps;

    /**
     * @return ARP configuration
     * 
     */
    public Optional<Output<List<VlanInterfaceArpArgs>>> arps() {
        return Optional.ofNullable(this.arps);
    }

    /**
     * Description
     * 
     */
    @Import(name="comment")
    private @Nullable Output<String> comment;

    /**
     * @return Description
     * 
     */
    public Optional<Output<String>> comment() {
        return Optional.ofNullable(this.comment);
    }

    /**
     * Dynamic DNS configuration specific to the Vlan Interfaces.
     * 
     */
    @Import(name="ddnsConfig")
    private @Nullable Output<VlanInterfaceDdnsConfigArgs> ddnsConfig;

    /**
     * @return Dynamic DNS configuration specific to the Vlan Interfaces.
     * 
     */
    public Optional<Output<VlanInterfaceDdnsConfigArgs>> ddnsConfig() {
        return Optional.ofNullable(this.ddnsConfig);
    }

    /**
     * Default interface assignment
     * 
     */
    @Import(name="defaultValue")
    private @Nullable Output<String> defaultValue;

    /**
     * @return Default interface assignment
     * 
     */
    public Optional<Output<String>> defaultValue() {
        return Optional.ofNullable(this.defaultValue);
    }

    /**
     * The device in which the resource is defined
     * &gt; ℹ️ **Note:** You must specify exactly one of `device`, `folder`, and `snippet`.
     * 
     */
    @Import(name="device")
    private @Nullable Output<String> device;

    /**
     * @return The device in which the resource is defined
     * &gt; ℹ️ **Note:** You must specify exactly one of `device`, `folder`, and `snippet`.
     * 
     */
    public Optional<Output<String>> device() {
        return Optional.ofNullable(this.device);
    }

    /**
     * Vlan interfaces DHCP Client Object
     * &gt; ℹ️ **Note:** You must specify exactly one of `dhcpClient` and `ip`.
     * 
     */
    @Import(name="dhcpClient")
    private @Nullable Output<VlanInterfaceDhcpClientArgs> dhcpClient;

    /**
     * @return Vlan interfaces DHCP Client Object
     * &gt; ℹ️ **Note:** You must specify exactly one of `dhcpClient` and `ip`.
     * 
     */
    public Optional<Output<VlanInterfaceDhcpClientArgs>> dhcpClient() {
        return Optional.ofNullable(this.dhcpClient);
    }

    /**
     * The folder in which the resource is defined
     * &gt; ℹ️ **Note:** You must specify exactly one of `device`, `folder`, and `snippet`.
     * 
     */
    @Import(name="folder")
    private @Nullable Output<String> folder;

    /**
     * @return The folder in which the resource is defined
     * &gt; ℹ️ **Note:** You must specify exactly one of `device`, `folder`, and `snippet`.
     * 
     */
    public Optional<Output<String>> folder() {
        return Optional.ofNullable(this.folder);
    }

    /**
     * Interface management profile
     * 
     */
    @Import(name="interfaceManagementProfile")
    private @Nullable Output<String> interfaceManagementProfile;

    /**
     * @return Interface management profile
     * 
     */
    public Optional<Output<String>> interfaceManagementProfile() {
        return Optional.ofNullable(this.interfaceManagementProfile);
    }

    /**
     * VLAN Interface IP Parent
     * &gt; ℹ️ **Note:** You must specify exactly one of `dhcpClient` and `ip`.
     * 
     */
    @Import(name="ips")
    private @Nullable Output<List<VlanInterfaceIpArgs>> ips;

    /**
     * @return VLAN Interface IP Parent
     * &gt; ℹ️ **Note:** You must specify exactly one of `dhcpClient` and `ip`.
     * 
     */
    public Optional<Output<List<VlanInterfaceIpArgs>>> ips() {
        return Optional.ofNullable(this.ips);
    }

    /**
     * MTU
     * 
     */
    @Import(name="mtu")
    private @Nullable Output<Integer> mtu;

    /**
     * @return MTU
     * 
     */
    public Optional<Output<Integer>> mtu() {
        return Optional.ofNullable(this.mtu);
    }

    /**
     * L3 sub-interface name
     * 
     */
    @Import(name="name")
    private @Nullable Output<String> name;

    /**
     * @return L3 sub-interface name
     * 
     */
    public Optional<Output<String>> name() {
        return Optional.ofNullable(this.name);
    }

    /**
     * The snippet in which the resource is defined
     * &gt; ℹ️ **Note:** You must specify exactly one of `device`, `folder`, and `snippet`.
     * 
     */
    @Import(name="snippet")
    private @Nullable Output<String> snippet;

    /**
     * @return The snippet in which the resource is defined
     * &gt; ℹ️ **Note:** You must specify exactly one of `device`, `folder`, and `snippet`.
     * 
     */
    public Optional<Output<String>> snippet() {
        return Optional.ofNullable(this.snippet);
    }

    @Import(name="tfid")
    private @Nullable Output<String> tfid;

    public Optional<Output<String>> tfid() {
        return Optional.ofNullable(this.tfid);
    }

    /**
     * VLAN tag
     * 
     */
    @Import(name="vlanTag")
    private @Nullable Output<String> vlanTag;

    /**
     * @return VLAN tag
     * 
     */
    public Optional<Output<String>> vlanTag() {
        return Optional.ofNullable(this.vlanTag);
    }

    private VlanInterfaceState() {}

    private VlanInterfaceState(VlanInterfaceState $) {
        this.arps = $.arps;
        this.comment = $.comment;
        this.ddnsConfig = $.ddnsConfig;
        this.defaultValue = $.defaultValue;
        this.device = $.device;
        this.dhcpClient = $.dhcpClient;
        this.folder = $.folder;
        this.interfaceManagementProfile = $.interfaceManagementProfile;
        this.ips = $.ips;
        this.mtu = $.mtu;
        this.name = $.name;
        this.snippet = $.snippet;
        this.tfid = $.tfid;
        this.vlanTag = $.vlanTag;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(VlanInterfaceState defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private VlanInterfaceState $;

        public Builder() {
            $ = new VlanInterfaceState();
        }

        public Builder(VlanInterfaceState defaults) {
            $ = new VlanInterfaceState(Objects.requireNonNull(defaults));
        }

        /**
         * @param arps ARP configuration
         * 
         * @return builder
         * 
         */
        public Builder arps(@Nullable Output<List<VlanInterfaceArpArgs>> arps) {
            $.arps = arps;
            return this;
        }

        /**
         * @param arps ARP configuration
         * 
         * @return builder
         * 
         */
        public Builder arps(List<VlanInterfaceArpArgs> arps) {
            return arps(Output.of(arps));
        }

        /**
         * @param arps ARP configuration
         * 
         * @return builder
         * 
         */
        public Builder arps(VlanInterfaceArpArgs... arps) {
            return arps(List.of(arps));
        }

        /**
         * @param comment Description
         * 
         * @return builder
         * 
         */
        public Builder comment(@Nullable Output<String> comment) {
            $.comment = comment;
            return this;
        }

        /**
         * @param comment Description
         * 
         * @return builder
         * 
         */
        public Builder comment(String comment) {
            return comment(Output.of(comment));
        }

        /**
         * @param ddnsConfig Dynamic DNS configuration specific to the Vlan Interfaces.
         * 
         * @return builder
         * 
         */
        public Builder ddnsConfig(@Nullable Output<VlanInterfaceDdnsConfigArgs> ddnsConfig) {
            $.ddnsConfig = ddnsConfig;
            return this;
        }

        /**
         * @param ddnsConfig Dynamic DNS configuration specific to the Vlan Interfaces.
         * 
         * @return builder
         * 
         */
        public Builder ddnsConfig(VlanInterfaceDdnsConfigArgs ddnsConfig) {
            return ddnsConfig(Output.of(ddnsConfig));
        }

        /**
         * @param defaultValue Default interface assignment
         * 
         * @return builder
         * 
         */
        public Builder defaultValue(@Nullable Output<String> defaultValue) {
            $.defaultValue = defaultValue;
            return this;
        }

        /**
         * @param defaultValue Default interface assignment
         * 
         * @return builder
         * 
         */
        public Builder defaultValue(String defaultValue) {
            return defaultValue(Output.of(defaultValue));
        }

        /**
         * @param device The device in which the resource is defined
         * &gt; ℹ️ **Note:** You must specify exactly one of `device`, `folder`, and `snippet`.
         * 
         * @return builder
         * 
         */
        public Builder device(@Nullable Output<String> device) {
            $.device = device;
            return this;
        }

        /**
         * @param device The device in which the resource is defined
         * &gt; ℹ️ **Note:** You must specify exactly one of `device`, `folder`, and `snippet`.
         * 
         * @return builder
         * 
         */
        public Builder device(String device) {
            return device(Output.of(device));
        }

        /**
         * @param dhcpClient Vlan interfaces DHCP Client Object
         * &gt; ℹ️ **Note:** You must specify exactly one of `dhcpClient` and `ip`.
         * 
         * @return builder
         * 
         */
        public Builder dhcpClient(@Nullable Output<VlanInterfaceDhcpClientArgs> dhcpClient) {
            $.dhcpClient = dhcpClient;
            return this;
        }

        /**
         * @param dhcpClient Vlan interfaces DHCP Client Object
         * &gt; ℹ️ **Note:** You must specify exactly one of `dhcpClient` and `ip`.
         * 
         * @return builder
         * 
         */
        public Builder dhcpClient(VlanInterfaceDhcpClientArgs dhcpClient) {
            return dhcpClient(Output.of(dhcpClient));
        }

        /**
         * @param folder The folder in which the resource is defined
         * &gt; ℹ️ **Note:** You must specify exactly one of `device`, `folder`, and `snippet`.
         * 
         * @return builder
         * 
         */
        public Builder folder(@Nullable Output<String> folder) {
            $.folder = folder;
            return this;
        }

        /**
         * @param folder The folder in which the resource is defined
         * &gt; ℹ️ **Note:** You must specify exactly one of `device`, `folder`, and `snippet`.
         * 
         * @return builder
         * 
         */
        public Builder folder(String folder) {
            return folder(Output.of(folder));
        }

        /**
         * @param interfaceManagementProfile Interface management profile
         * 
         * @return builder
         * 
         */
        public Builder interfaceManagementProfile(@Nullable Output<String> interfaceManagementProfile) {
            $.interfaceManagementProfile = interfaceManagementProfile;
            return this;
        }

        /**
         * @param interfaceManagementProfile Interface management profile
         * 
         * @return builder
         * 
         */
        public Builder interfaceManagementProfile(String interfaceManagementProfile) {
            return interfaceManagementProfile(Output.of(interfaceManagementProfile));
        }

        /**
         * @param ips VLAN Interface IP Parent
         * &gt; ℹ️ **Note:** You must specify exactly one of `dhcpClient` and `ip`.
         * 
         * @return builder
         * 
         */
        public Builder ips(@Nullable Output<List<VlanInterfaceIpArgs>> ips) {
            $.ips = ips;
            return this;
        }

        /**
         * @param ips VLAN Interface IP Parent
         * &gt; ℹ️ **Note:** You must specify exactly one of `dhcpClient` and `ip`.
         * 
         * @return builder
         * 
         */
        public Builder ips(List<VlanInterfaceIpArgs> ips) {
            return ips(Output.of(ips));
        }

        /**
         * @param ips VLAN Interface IP Parent
         * &gt; ℹ️ **Note:** You must specify exactly one of `dhcpClient` and `ip`.
         * 
         * @return builder
         * 
         */
        public Builder ips(VlanInterfaceIpArgs... ips) {
            return ips(List.of(ips));
        }

        /**
         * @param mtu MTU
         * 
         * @return builder
         * 
         */
        public Builder mtu(@Nullable Output<Integer> mtu) {
            $.mtu = mtu;
            return this;
        }

        /**
         * @param mtu MTU
         * 
         * @return builder
         * 
         */
        public Builder mtu(Integer mtu) {
            return mtu(Output.of(mtu));
        }

        /**
         * @param name L3 sub-interface name
         * 
         * @return builder
         * 
         */
        public Builder name(@Nullable Output<String> name) {
            $.name = name;
            return this;
        }

        /**
         * @param name L3 sub-interface name
         * 
         * @return builder
         * 
         */
        public Builder name(String name) {
            return name(Output.of(name));
        }

        /**
         * @param snippet The snippet in which the resource is defined
         * &gt; ℹ️ **Note:** You must specify exactly one of `device`, `folder`, and `snippet`.
         * 
         * @return builder
         * 
         */
        public Builder snippet(@Nullable Output<String> snippet) {
            $.snippet = snippet;
            return this;
        }

        /**
         * @param snippet The snippet in which the resource is defined
         * &gt; ℹ️ **Note:** You must specify exactly one of `device`, `folder`, and `snippet`.
         * 
         * @return builder
         * 
         */
        public Builder snippet(String snippet) {
            return snippet(Output.of(snippet));
        }

        public Builder tfid(@Nullable Output<String> tfid) {
            $.tfid = tfid;
            return this;
        }

        public Builder tfid(String tfid) {
            return tfid(Output.of(tfid));
        }

        /**
         * @param vlanTag VLAN tag
         * 
         * @return builder
         * 
         */
        public Builder vlanTag(@Nullable Output<String> vlanTag) {
            $.vlanTag = vlanTag;
            return this;
        }

        /**
         * @param vlanTag VLAN tag
         * 
         * @return builder
         * 
         */
        public Builder vlanTag(String vlanTag) {
            return vlanTag(Output.of(vlanTag));
        }

        public VlanInterfaceState build() {
            return $;
        }
    }

}
