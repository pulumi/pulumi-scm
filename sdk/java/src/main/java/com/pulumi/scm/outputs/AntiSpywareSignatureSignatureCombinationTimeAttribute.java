// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.scm.outputs;

import com.pulumi.core.annotations.CustomType;
import java.lang.Integer;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;

@CustomType
public final class AntiSpywareSignatureSignatureCombinationTimeAttribute {
    /**
     * @return The Interval param. Value must be between 1 and 3600.
     * 
     */
    private @Nullable Integer interval;
    /**
     * @return The Threshold param. Value must be between 1 and 255.
     * 
     */
    private @Nullable Integer threshold;
    /**
     * @return The TrackBy param. String must be one of these: `&#34;source-and-destination&#34;`, `&#34;source&#34;`, `&#34;destination&#34;`.
     * 
     */
    private @Nullable String trackBy;

    private AntiSpywareSignatureSignatureCombinationTimeAttribute() {}
    /**
     * @return The Interval param. Value must be between 1 and 3600.
     * 
     */
    public Optional<Integer> interval() {
        return Optional.ofNullable(this.interval);
    }
    /**
     * @return The Threshold param. Value must be between 1 and 255.
     * 
     */
    public Optional<Integer> threshold() {
        return Optional.ofNullable(this.threshold);
    }
    /**
     * @return The TrackBy param. String must be one of these: `&#34;source-and-destination&#34;`, `&#34;source&#34;`, `&#34;destination&#34;`.
     * 
     */
    public Optional<String> trackBy() {
        return Optional.ofNullable(this.trackBy);
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(AntiSpywareSignatureSignatureCombinationTimeAttribute defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private @Nullable Integer interval;
        private @Nullable Integer threshold;
        private @Nullable String trackBy;
        public Builder() {}
        public Builder(AntiSpywareSignatureSignatureCombinationTimeAttribute defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.interval = defaults.interval;
    	      this.threshold = defaults.threshold;
    	      this.trackBy = defaults.trackBy;
        }

        @CustomType.Setter
        public Builder interval(@Nullable Integer interval) {

            this.interval = interval;
            return this;
        }
        @CustomType.Setter
        public Builder threshold(@Nullable Integer threshold) {

            this.threshold = threshold;
            return this;
        }
        @CustomType.Setter
        public Builder trackBy(@Nullable String trackBy) {

            this.trackBy = trackBy;
            return this;
        }
        public AntiSpywareSignatureSignatureCombinationTimeAttribute build() {
            final var _resultValue = new AntiSpywareSignatureSignatureCombinationTimeAttribute();
            _resultValue.interval = interval;
            _resultValue.threshold = threshold;
            _resultValue.trackBy = trackBy;
            return _resultValue;
        }
    }
}
