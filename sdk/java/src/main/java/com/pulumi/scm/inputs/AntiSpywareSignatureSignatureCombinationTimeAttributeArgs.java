// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.scm.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import java.lang.Integer;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class AntiSpywareSignatureSignatureCombinationTimeAttributeArgs extends com.pulumi.resources.ResourceArgs {

    public static final AntiSpywareSignatureSignatureCombinationTimeAttributeArgs Empty = new AntiSpywareSignatureSignatureCombinationTimeAttributeArgs();

    /**
     * The Interval param. Value must be between 1 and 3600.
     * 
     */
    @Import(name="interval")
    private @Nullable Output<Integer> interval;

    /**
     * @return The Interval param. Value must be between 1 and 3600.
     * 
     */
    public Optional<Output<Integer>> interval() {
        return Optional.ofNullable(this.interval);
    }

    /**
     * The Threshold param. Value must be between 1 and 255.
     * 
     */
    @Import(name="threshold")
    private @Nullable Output<Integer> threshold;

    /**
     * @return The Threshold param. Value must be between 1 and 255.
     * 
     */
    public Optional<Output<Integer>> threshold() {
        return Optional.ofNullable(this.threshold);
    }

    /**
     * The TrackBy param. String must be one of these: `&#34;source-and-destination&#34;`, `&#34;source&#34;`, `&#34;destination&#34;`.
     * 
     */
    @Import(name="trackBy")
    private @Nullable Output<String> trackBy;

    /**
     * @return The TrackBy param. String must be one of these: `&#34;source-and-destination&#34;`, `&#34;source&#34;`, `&#34;destination&#34;`.
     * 
     */
    public Optional<Output<String>> trackBy() {
        return Optional.ofNullable(this.trackBy);
    }

    private AntiSpywareSignatureSignatureCombinationTimeAttributeArgs() {}

    private AntiSpywareSignatureSignatureCombinationTimeAttributeArgs(AntiSpywareSignatureSignatureCombinationTimeAttributeArgs $) {
        this.interval = $.interval;
        this.threshold = $.threshold;
        this.trackBy = $.trackBy;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(AntiSpywareSignatureSignatureCombinationTimeAttributeArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private AntiSpywareSignatureSignatureCombinationTimeAttributeArgs $;

        public Builder() {
            $ = new AntiSpywareSignatureSignatureCombinationTimeAttributeArgs();
        }

        public Builder(AntiSpywareSignatureSignatureCombinationTimeAttributeArgs defaults) {
            $ = new AntiSpywareSignatureSignatureCombinationTimeAttributeArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param interval The Interval param. Value must be between 1 and 3600.
         * 
         * @return builder
         * 
         */
        public Builder interval(@Nullable Output<Integer> interval) {
            $.interval = interval;
            return this;
        }

        /**
         * @param interval The Interval param. Value must be between 1 and 3600.
         * 
         * @return builder
         * 
         */
        public Builder interval(Integer interval) {
            return interval(Output.of(interval));
        }

        /**
         * @param threshold The Threshold param. Value must be between 1 and 255.
         * 
         * @return builder
         * 
         */
        public Builder threshold(@Nullable Output<Integer> threshold) {
            $.threshold = threshold;
            return this;
        }

        /**
         * @param threshold The Threshold param. Value must be between 1 and 255.
         * 
         * @return builder
         * 
         */
        public Builder threshold(Integer threshold) {
            return threshold(Output.of(threshold));
        }

        /**
         * @param trackBy The TrackBy param. String must be one of these: `&#34;source-and-destination&#34;`, `&#34;source&#34;`, `&#34;destination&#34;`.
         * 
         * @return builder
         * 
         */
        public Builder trackBy(@Nullable Output<String> trackBy) {
            $.trackBy = trackBy;
            return this;
        }

        /**
         * @param trackBy The TrackBy param. String must be one of these: `&#34;source-and-destination&#34;`, `&#34;source&#34;`, `&#34;destination&#34;`.
         * 
         * @return builder
         * 
         */
        public Builder trackBy(String trackBy) {
            return trackBy(Output.of(trackBy));
        }

        public AntiSpywareSignatureSignatureCombinationTimeAttributeArgs build() {
            return $;
        }
    }

}
