// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.scm.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import com.pulumi.scm.inputs.BgpRouteMapRedistributionBgpRibRouteMapMatchIpv4Args;
import java.lang.Integer;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class BgpRouteMapRedistributionBgpRibRouteMapMatchArgs extends com.pulumi.resources.ResourceArgs {

    public static final BgpRouteMapRedistributionBgpRibRouteMapMatchArgs Empty = new BgpRouteMapRedistributionBgpRibRouteMapMatchArgs();

    /**
     * BGP Root RIB Route maps match AS path access list
     * 
     */
    @Import(name="asPathAccessList")
    private @Nullable Output<String> asPathAccessList;

    /**
     * @return BGP Root RIB Route maps match AS path access list
     * 
     */
    public Optional<Output<String>> asPathAccessList() {
        return Optional.ofNullable(this.asPathAccessList);
    }

    /**
     * BGP Root RIB Route maps match Extended community
     * 
     */
    @Import(name="extendedCommunity")
    private @Nullable Output<String> extendedCommunity;

    /**
     * @return BGP Root RIB Route maps match Extended community
     * 
     */
    public Optional<Output<String>> extendedCommunity() {
        return Optional.ofNullable(this.extendedCommunity);
    }

    /**
     * BGP Root RIB Route maps match Interface
     * 
     */
    @Import(name="interface")
    private @Nullable Output<String> interface_;

    /**
     * @return BGP Root RIB Route maps match Interface
     * 
     */
    public Optional<Output<String>> interface_() {
        return Optional.ofNullable(this.interface_);
    }

    /**
     * BGP Route Map Redistributions Root BGP rib Route Map IPv4
     * 
     */
    @Import(name="ipv4")
    private @Nullable Output<BgpRouteMapRedistributionBgpRibRouteMapMatchIpv4Args> ipv4;

    /**
     * @return BGP Route Map Redistributions Root BGP rib Route Map IPv4
     * 
     */
    public Optional<Output<BgpRouteMapRedistributionBgpRibRouteMapMatchIpv4Args>> ipv4() {
        return Optional.ofNullable(this.ipv4);
    }

    /**
     * BGP Root RIB Route maps match Large community
     * 
     */
    @Import(name="largeCommunity")
    private @Nullable Output<String> largeCommunity;

    /**
     * @return BGP Root RIB Route maps match Large community
     * 
     */
    public Optional<Output<String>> largeCommunity() {
        return Optional.ofNullable(this.largeCommunity);
    }

    /**
     * BGP Root RIB Route maps match Local preference
     * 
     */
    @Import(name="localPreference")
    private @Nullable Output<Integer> localPreference;

    /**
     * @return BGP Root RIB Route maps match Local preference
     * 
     */
    public Optional<Output<Integer>> localPreference() {
        return Optional.ofNullable(this.localPreference);
    }

    /**
     * BGP Root RIB Route maps match Metric
     * 
     */
    @Import(name="metric")
    private @Nullable Output<Integer> metric;

    /**
     * @return BGP Root RIB Route maps match Metric
     * 
     */
    public Optional<Output<Integer>> metric() {
        return Optional.ofNullable(this.metric);
    }

    /**
     * BGP Root RIB Route maps match Origin
     * 
     */
    @Import(name="origin")
    private @Nullable Output<String> origin;

    /**
     * @return BGP Root RIB Route maps match Origin
     * 
     */
    public Optional<Output<String>> origin() {
        return Optional.ofNullable(this.origin);
    }

    /**
     * BGP Root RIB Route maps match Peer
     * 
     */
    @Import(name="peer")
    private @Nullable Output<String> peer;

    /**
     * @return BGP Root RIB Route maps match Peer
     * 
     */
    public Optional<Output<String>> peer() {
        return Optional.ofNullable(this.peer);
    }

    /**
     * BGP Root RIB Route maps match Regular community
     * 
     */
    @Import(name="regularCommunity")
    private @Nullable Output<String> regularCommunity;

    /**
     * @return BGP Root RIB Route maps match Regular community
     * 
     */
    public Optional<Output<String>> regularCommunity() {
        return Optional.ofNullable(this.regularCommunity);
    }

    /**
     * BGP Root RIB Route maps match Tag
     * 
     */
    @Import(name="tag")
    private @Nullable Output<Integer> tag;

    /**
     * @return BGP Root RIB Route maps match Tag
     * 
     */
    public Optional<Output<Integer>> tag() {
        return Optional.ofNullable(this.tag);
    }

    private BgpRouteMapRedistributionBgpRibRouteMapMatchArgs() {}

    private BgpRouteMapRedistributionBgpRibRouteMapMatchArgs(BgpRouteMapRedistributionBgpRibRouteMapMatchArgs $) {
        this.asPathAccessList = $.asPathAccessList;
        this.extendedCommunity = $.extendedCommunity;
        this.interface_ = $.interface_;
        this.ipv4 = $.ipv4;
        this.largeCommunity = $.largeCommunity;
        this.localPreference = $.localPreference;
        this.metric = $.metric;
        this.origin = $.origin;
        this.peer = $.peer;
        this.regularCommunity = $.regularCommunity;
        this.tag = $.tag;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(BgpRouteMapRedistributionBgpRibRouteMapMatchArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private BgpRouteMapRedistributionBgpRibRouteMapMatchArgs $;

        public Builder() {
            $ = new BgpRouteMapRedistributionBgpRibRouteMapMatchArgs();
        }

        public Builder(BgpRouteMapRedistributionBgpRibRouteMapMatchArgs defaults) {
            $ = new BgpRouteMapRedistributionBgpRibRouteMapMatchArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param asPathAccessList BGP Root RIB Route maps match AS path access list
         * 
         * @return builder
         * 
         */
        public Builder asPathAccessList(@Nullable Output<String> asPathAccessList) {
            $.asPathAccessList = asPathAccessList;
            return this;
        }

        /**
         * @param asPathAccessList BGP Root RIB Route maps match AS path access list
         * 
         * @return builder
         * 
         */
        public Builder asPathAccessList(String asPathAccessList) {
            return asPathAccessList(Output.of(asPathAccessList));
        }

        /**
         * @param extendedCommunity BGP Root RIB Route maps match Extended community
         * 
         * @return builder
         * 
         */
        public Builder extendedCommunity(@Nullable Output<String> extendedCommunity) {
            $.extendedCommunity = extendedCommunity;
            return this;
        }

        /**
         * @param extendedCommunity BGP Root RIB Route maps match Extended community
         * 
         * @return builder
         * 
         */
        public Builder extendedCommunity(String extendedCommunity) {
            return extendedCommunity(Output.of(extendedCommunity));
        }

        /**
         * @param interface_ BGP Root RIB Route maps match Interface
         * 
         * @return builder
         * 
         */
        public Builder interface_(@Nullable Output<String> interface_) {
            $.interface_ = interface_;
            return this;
        }

        /**
         * @param interface_ BGP Root RIB Route maps match Interface
         * 
         * @return builder
         * 
         */
        public Builder interface_(String interface_) {
            return interface_(Output.of(interface_));
        }

        /**
         * @param ipv4 BGP Route Map Redistributions Root BGP rib Route Map IPv4
         * 
         * @return builder
         * 
         */
        public Builder ipv4(@Nullable Output<BgpRouteMapRedistributionBgpRibRouteMapMatchIpv4Args> ipv4) {
            $.ipv4 = ipv4;
            return this;
        }

        /**
         * @param ipv4 BGP Route Map Redistributions Root BGP rib Route Map IPv4
         * 
         * @return builder
         * 
         */
        public Builder ipv4(BgpRouteMapRedistributionBgpRibRouteMapMatchIpv4Args ipv4) {
            return ipv4(Output.of(ipv4));
        }

        /**
         * @param largeCommunity BGP Root RIB Route maps match Large community
         * 
         * @return builder
         * 
         */
        public Builder largeCommunity(@Nullable Output<String> largeCommunity) {
            $.largeCommunity = largeCommunity;
            return this;
        }

        /**
         * @param largeCommunity BGP Root RIB Route maps match Large community
         * 
         * @return builder
         * 
         */
        public Builder largeCommunity(String largeCommunity) {
            return largeCommunity(Output.of(largeCommunity));
        }

        /**
         * @param localPreference BGP Root RIB Route maps match Local preference
         * 
         * @return builder
         * 
         */
        public Builder localPreference(@Nullable Output<Integer> localPreference) {
            $.localPreference = localPreference;
            return this;
        }

        /**
         * @param localPreference BGP Root RIB Route maps match Local preference
         * 
         * @return builder
         * 
         */
        public Builder localPreference(Integer localPreference) {
            return localPreference(Output.of(localPreference));
        }

        /**
         * @param metric BGP Root RIB Route maps match Metric
         * 
         * @return builder
         * 
         */
        public Builder metric(@Nullable Output<Integer> metric) {
            $.metric = metric;
            return this;
        }

        /**
         * @param metric BGP Root RIB Route maps match Metric
         * 
         * @return builder
         * 
         */
        public Builder metric(Integer metric) {
            return metric(Output.of(metric));
        }

        /**
         * @param origin BGP Root RIB Route maps match Origin
         * 
         * @return builder
         * 
         */
        public Builder origin(@Nullable Output<String> origin) {
            $.origin = origin;
            return this;
        }

        /**
         * @param origin BGP Root RIB Route maps match Origin
         * 
         * @return builder
         * 
         */
        public Builder origin(String origin) {
            return origin(Output.of(origin));
        }

        /**
         * @param peer BGP Root RIB Route maps match Peer
         * 
         * @return builder
         * 
         */
        public Builder peer(@Nullable Output<String> peer) {
            $.peer = peer;
            return this;
        }

        /**
         * @param peer BGP Root RIB Route maps match Peer
         * 
         * @return builder
         * 
         */
        public Builder peer(String peer) {
            return peer(Output.of(peer));
        }

        /**
         * @param regularCommunity BGP Root RIB Route maps match Regular community
         * 
         * @return builder
         * 
         */
        public Builder regularCommunity(@Nullable Output<String> regularCommunity) {
            $.regularCommunity = regularCommunity;
            return this;
        }

        /**
         * @param regularCommunity BGP Root RIB Route maps match Regular community
         * 
         * @return builder
         * 
         */
        public Builder regularCommunity(String regularCommunity) {
            return regularCommunity(Output.of(regularCommunity));
        }

        /**
         * @param tag BGP Root RIB Route maps match Tag
         * 
         * @return builder
         * 
         */
        public Builder tag(@Nullable Output<Integer> tag) {
            $.tag = tag;
            return this;
        }

        /**
         * @param tag BGP Root RIB Route maps match Tag
         * 
         * @return builder
         * 
         */
        public Builder tag(Integer tag) {
            return tag(Output.of(tag));
        }

        public BgpRouteMapRedistributionBgpRibRouteMapMatchArgs build() {
            return $;
        }
    }

}
